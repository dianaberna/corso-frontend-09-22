...
btnGenerator.addEventListener("click", function cambiaNome() {
  const poeti = Object.keys(poesie);
  const poesieTot = Object.values(poesie);
  const numeroPoeta = Math.floor(Math.random() * poeti.length);
  console.log(numeroPoeta);

  const poeta = poeti[numeroPoeta];
  const poesia = poesieTot[numeroPoeta];

  // document.getElementById("poesia").textContent = poesia;
  document.getElementById("poeta").textContent = poeta;
  document.getElementById("poesia").textContent = poesia;
});
