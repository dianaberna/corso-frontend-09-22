...
/* 
function getLettersFromNumbers(aNumber){
    
    let str1="invalid";
    let stringa="abcdefghilmnopqrstuvz";
    aNumber--;

    if(aNumber>=0 && aNumber<=20)
    {
        let cod=stringa.charCodeAt(aNumber);
        str1=String.fromCharCode(cod);
    }
    
    console.log(`Al numero ${++aNumber} corrisponde la lettera:${str1}`)
    return str1;

}

getLettersFromNumbers(prompt("Inserisci un numero da 1 a 21"));

 */


/* function scontato(prezzo, sconto){
    
    let prezzofinale = (prezzo-((prezzo/100) * sconto));
    console.log(prezzofinale);
    return prezzofinale.toFixed(2);
}


console.log("Il prezzo dopo lo sconto è "+scontato(prompt("Inserisci il prezzo pieno:"),prompt("Inserisci la percentuale di sconto:"))+"€"); 
*/

/* 
function yourOp(fat1, op, fat2){
    ...
    switch(op){
        ...
        case ":":
            if(fat2!==0){
                res=fat1/fat2;
            }else{res="impossibile dividere per 0!!"}
            break;
        case "/":
            if(fat2!==0){
                res=fat1/fat2;
            ...
            break;
        default:
            res="Operatore non riconosciuto."
    }
    return res;
}

console.log("Il risultato è "+yourOp(prompt("Inserisci il primo fattore:"),prompt("Inserisci l'operatore:"),prompt("Inserisci il secondo fattore:")));
*/


/* function sommaArray(anArray){
    let sum=0;
...
let myArray = [2,3,4,10];
console.log(`La somma dell'array è ${sommaArray(myArray)}`);
 */

/* 
function quantiTrue(anArray){
    let sum=0;
    for(let i=0; i<anArray.length; i++){
        if(anArray[i]==true){sum++};
    }
    return "Trovi nell'array "+sum+" TRUE su "+anArray.length+" elementi.";
}

let myArray = [true,false,false,true,false,true];
console.log(quantiTrue(myArray));

 */

/* 
    let exp=prompt("Quante volte piego (da 1 in poi)?");
    let res= ((0.5*(Math.pow(2,exp)-1))+0.5)/1000;
    console.log("Dopo la piega la carta sarà spessa: "+res+"m");
     */
...
function validateEmail()
{
    ...
    let testEmails = prompt("Mail:");//["notanemail.com", "workingexample@email.com", "another_working@somethingelse.org", "notworking@1.com"];
...
function sassoCartaForbici(p1, p2) {
    return p1 == p2 ? `It's a draw` 
        : p1 == `Rock` && p2 == "Scissors" ? `The winner is p1` : p1 == `Rock` ? `The winner is p2` 
...
function quantiGiorni(primaData, secondaData){
    ...
    console.log("unGiorno: "+unGiorno+" prima data: "+primaData+" seconda data:"+secondaData);
...
function preventivo(listaSpesa){
    let tot = 0;
    console.log(listaSpesa.lenght);
    for(let i=0; i<listaSpesa.lenght; i++)
    {
        tot= tot+listaSpesa[i].prezzo;
    }
    return tot;
}

console.log(preventivo([
    { prodotto: "pane", qta: 1, prezzo: 4 },
    { prodotto: "cereali", qta: 5, prezzo: 2.7 },
    { prodotto: "vino", qta: 2, prezzo: 6 },
]));